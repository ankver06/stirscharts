...
const chartFiles = {
  outright: "sofr/outright.json",
  spread3m: "sofr/3m_spread.json",
  spread6m: "sofr/6m_spread.json",
  spread12m: "sofr/12m_spread.json",
  fly3m: "sofr/3m_fly.json",
  fly6m: "sofr/6m_fly.json",
  fly12m: "sofr/12m_fly.json",
  dfly3m: "sofr/3m_double_fly.json",
  dfly6m: "sofr/6m_double_fly.json",
  dfly12m: "sofr/12m_double_fly.json"
};
...
function loadChart(filePath, chartKey) {
  fetch(filePath)
    ...
    .then(data => {
      ...
      titleEl.textContent = `SOFR ${prettyLabel} Curve`;

      const labels = data.map(point => point.Contract);
      const values = data.map(point => point.Price);

      const ctx = document.getElementById('sofrChart').getContext('2d');
...
loadChart('data/sofr/outright.json', 'outright');
